...
def inject_sidebar_toggle():
    """
    Injects Material Icons font into the top-level document head so that
    Streamlit's sidebar toggle ligature (keyboard_double_arrow_left) renders
    as the actual icon instead of raw text. Uses window.parent to escape
...
(function() {
  function injectFont(doc) {
    if (!doc) return;
    if (doc.getElementById('material-icons-font')) return;
  ...
  // Inject into current document and parent (Streamlit iframe escape)
  try { injectFont(document); } catch(e) {}
  try { injectFont(window.parent.document); } catch(e) {}
  try { injectFont(window.top.document); } catch(e) {}

  // Retry after Streamlit finishes mounting
  setTimeout(function() {
    try { injectFont(window.parent.document); } catch(e) {}
    try { injectFont(window.top.document); } catch(e) {}
  }, 800);
